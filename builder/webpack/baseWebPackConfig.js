...
/**
 * separate function to get the config so we can call it from environment 
 based config files.
 */
export const getBaseWebPackConfig = (env, argv) => {
  let config = {}
  ...
  config.module = {
    rules: [
      ...
      },
      {
        ...
        test: /\.(woff(2)?|ttf|eot|svg|jpg|jpeg|png|gif|pdf)(\?v=\d+\.\d+\.\d+)?$/, //here we tell webpack that all fonts, images, pdfs are asset/resource
        type: 'asset/resource',
      },
      {
        ...
        use: [
          //use tells this rule what loaders to use, loaders are used in a last to first order.  So the last loader is processed first, then the loader above it, till the first loader.
          {
